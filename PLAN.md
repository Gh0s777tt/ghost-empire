...
4. 🟡 **Code-split `AdminClient.tsx`** — ✅ `@next/bundle-analyzer` wpięty (`npm run analyze` → treemapy w `.next/analyze`, `cross-env` pod Windows; produkcyjny `next build` przechodzi lokalnie). **Zostaje** sam split monolitu (~7k linii): `/admin` to route on-demand (nie obciąża bundla publicznego — niski priorytet), a rozbicie wymaga najpierw wyniesienia inline'owego `SectionCard` + typów do współdzielonego modułu (większy refaktor; zrobić jako osobny mierzalny PR z podglądem `analyze`).
