...
4. 🟡 **Code-split `AdminClient.tsx`** — ✅ `@next/bundle-analyzer` (`npm run analyze`) + ✅ **start splitu**: `SectionCard`→`admin/shared.tsx`, `AuditEntry`→`admin/types.ts`, sekcje **Analityka** + **Audit log** wycięte do `admin/sections/` i `next/dynamic` (`ssr:false`). Wzorzec gotowy. **Zostaje:** przenosić kolejne (cięższe) sekcje tym samym wzorcem (Shop/Seasons/StreamAlerts/Predictions/Goals/Users/chat-suite…) — iteracyjnie, niski priorytet (`/admin` = route on-demand).
