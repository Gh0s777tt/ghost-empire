...
> - 🏆 **Osiągnięcia za prestiż/pojedynki/kasyno** (#179) — +7 achievementów (60 łącznie), triggery `prestige`/`duels_won`/`casino_plays`, fire-and-forget. **⚠️ aktywacja na prodzie: `npm run db:seed:achievements`** (NIE pełny `db:seed` — kasuje sklep/eventy). ✅ `tsc`/`next build`/135 testów. **[zrobione na prodzie — 60 osiągnięć żyje]**
> - 🔊 **TTS na overlayu alertów** (#180) — czytanie alertów na głos przez `speechSynthesis` przeglądarki (działa w OBS, za darmo); włączane `&tts=1` w URL źródła (+`&ttsTypes/ttsRate/ttsVolume/ttsVoice`). Czysto klientowe, **bez schematu/db push**. ✅ `tsc`/`next build`/135 testów.
> - 🏦 **Mini-gra kooperacyjna `!heist`** (#181) — napad na GT (okno 90 s, zbiorowy rzut, szansa rośnie z ekipą 30%→60%, sukces 2× / wpadka strata). Escrow + atomowa wypłata (`lib/heist.ts`), modele `Heist`/`HeistEntry` (`db push`), portal `/api/bot/heist`, bot `heist.ts` ×3 ze schedulerem (`broadcast.sendTo`). +5 testów = 140. ⚠️ **db push + restart bota.** ✅ web `tsc`/`next build`/testy, bot `tsc`.
> - 🎡 **Mini-gra `!roulette`** (#182) — europejska ruletka 0–36 (red/black 2× / liczba 36×, house edge z zera), rozszerza `playGtGame` o `choice`, **bez nowego modelu (bez db push)**, chat `!roulette`/`!roleta` + web `/kasyno`. +4 testy = 144. ⚠️ **restart bota.** ✅ web `tsc`/`next build`/testy, bot `tsc`.
> - 🧭 **Górny pasek — grupowana nawigacja** (#183) — desktop: 3 bezpośrednie + 2 rozwijane grupy (GRY/SPOŁECZNOŚĆ, hover+focus-within, czysto CSS) zamiast 11 płaskich linków (brakowało miejsca). PROFIL → menu avatara. Mobile = płaski scroll bez zmian. `Header.tsx`. ✅ `tsc`/`next build`. *(Pierwszy kęs „🎨 redesign" — reszta gustu czeka na usera.)*
> - 💬 **Discord → osobny `E-Bot`, `ghost-empire-bot` zastąpiony** (#184, docs) — finalny podział: **`ghost-empire-chat` = streaming**, **`E-Bot` (`Gh0s777tt/E-Bot`) = Discord + społeczność**. Ekonomia Discord (GT za wiadomości/voice) + linkowanie przeniesione do E-Bota — woła **te same** endpointy portalu (`/api/internal/award`, `/link-discord`, polling `/api/bot/config`), **kontrakt niezmieniony**. Integracja E-Bota domknięta w jego PR #1 (`empire/award|config|messages|voice.mts`, `/portal`, warunkowe privileged intents, opt-in `GHOST_ECONOMY=1`). Ten PR = **tylko docs GE** (banner deprecated w `ghost-empire-bot/README.md` + sync README/ARCHITECTURE/ENV/ENDPOINTS/PHASE3/on-site about). Bez zmian kodu portalu, bez `db push`. **⚠️ Akcje usera:** włącz ekonomię w E-Bocie (env + intencje + `npm run deploy` + restart) i **wyłącz stary `ghost-empire-bot`** (inaczej podwójne GT).
...
6. ~~**i18n PL/EN — fundament**~~ → ✅ **DONE: 14 lokalizacji UI** (PL/EN/DE/ES/IT/FR/RU/UK/ZH/JA/KO/AR/PT/ID, AR=RTL) = po 1963 klucze = 100%.
