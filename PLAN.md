...
> - 🏆 **Osiągnięcia za prestiż/pojedynki/kasyno** (#179) — +7 achievementów (60 łącznie), triggery `prestige`/`duels_won`/`casino_plays`, fire-and-forget. **⚠️ aktywacja na prodzie: `npm run db:seed:achievements`** (NIE pełny `db:seed` — kasuje sklep/eventy). ✅ `tsc`/`next build`/135 testów.
