...
| `SUPABASE_URL` (O) | **Upload mediów** (własne tło / grafiki+animacje alertów / elementy scen → Supabase Storage). URL projektu, np. `https://<ref>.supabase.co`. Bez tej i `SUPABASE_SERVICE_ROLE_KEY` endpoint `/api/upload` zwraca 503 (dry-wired). **Wymaga też publicznego bucketa** (patrz `MEDIA_BUCKET`). | Supabase → Settings → API → Project URL |
| `SUPABASE_SERVICE_ROLE_KEY` (O) | Klucz service-role do zapisu w Storage (`/api/upload`). **Sekret** — tylko serwer. | Supabase → Settings → API → service_role |
| `MEDIA_BUCKET` (O) | Nazwa **publicznego** bucketa Storage na media (domyślnie `media`). Utwórz go w Supabase → Storage → New bucket → **Public**. Upload trafia pod `<tenantId>/<uuid>.<ext>`; publiczny URL serwuje Supabase. Dozwolone: PNG/JPG/GIF/WebP/AVIF/MP4/WebM (SVG wykluczony — XSS w źródle OBS), limit 20 MB. | (domyślnie `media`) |
| `BOT_SECRET` | Bearer dla `/api/internal/*` i `/api/bot/*` — **ten sam** w obu botach. To **globalny fallback**: akceptowany dla **każdego** portalu. Portal może dodatkowo mieć **własny** sekret (`Tenant.botSecret`) generowany w `/admin#bot` — wtedy jego instancja bota używa tamtego, a ten dalej działa — chyba że włączysz `BOT_SECRET_STRICT` (niżej), które odbiera globalnemu status wytrycha do portali z własnym sekretem. Zob. [PER-TENANT-IDENTITY §9](PER-TENANT-IDENTITY.md#10-per-tenant-bot-identity-tenantbotsecret) | `openssl rand -hex 32` |
| `BOT_SECRET_STRICT` (O) | **Izolacja sekretu bota między portalami.** Domyślnie (brak zmiennej) globalny `BOT_SECRET` jest akceptowany dla **każdego** portalu — a że tenant wynika z nagłówka Host, a `/api` jest poza middleware, posiadacz jednego globalnego sekretu działa w **cudzym** portalu (mennica waluty, tożsamości, kasyno) i rotacja `Tenant.botSecret` nic mu nie odbiera. Ustaw `1` lub `true`, żeby dla portali z **własnym** sekretem akceptowany był **tylko** ich sekret; portale **bez** własnego sekretu nadal używają globalnego (inaczej zgasłyby w chwili włączenia flagi). Cokolwiek innego (`0`, `false`, pusty) = tryb zgodnościowy — literówka nie odetnie bota po cichu. **Kiedy przestawić:** gdy każda instancja bota ma już wklejony sekret swojego portalu (`/admin#bot` → „Rotuj sekret" → `tenants/<slug>.env`). Trade-off: włączone = izolacja, ale instancja bota jadąca na globalnym sekrecie do portalu z własnym dostanie 401 | `1` (po rotacji wszystkich portali) |
...
| `RUMBLE_API_URL` (O) | **Globalny fallback** dla statusu/overlayu Rumble (`/overlay/rumble`). Od #730 każdy portal ustawia **własny** Rumble Livestream API URL w `/admin#rumble` (`IntegrationConfig.rumbleApiUrl`, szyfrowany at-rest) — ten env to już tylko fallback foundera, gdy portal nie ma własnego. Ustawione w Vercelu |
...
| `META_IG_TOKEN` (O) | **Globalny fallback** tokenu Meta/Instagram do auto-postów (`lib/meta-social.ts`). Produkcyjnie ustawiany per-portal w panelu integracji (`metaIgToken`, szyfrowany at-rest) — ten env to fallback foundera, gdy portal nie ma własnego |
| `X_API_TOKEN` (O) | **Globalny fallback** tokenu X (Twitter) do auto-postów (`lib/x-social.ts`). Produkcyjnie ustawiany per-portal w panelu integracji (`xApiToken`, szyfrowany at-rest) — ten env to fallback foundera, gdy portal nie ma własnego |
| `CRON_SECRET` | Bearer chroniący crony Vercel: `/api/cron/streamlabs-poll` (polling donacji), `/api/cron/prune` (czyszczenie starych rekordów), `/api/cron/weekly-rewards` (poniedziałki 00:00 — nagrody rankingu tygodnia per portal), `/api/cron/backup` (05:00 — off-site backup) i `/api/cron/weekly-digest` (poniedziałki 07:00 — tygodniowy raport email do właścicieli portali, #773) |
| `RESEND_API_KEY` / `EMAIL_FROM` (O) | **Dormant** email transakcyjny. `RESEND_API_KEY` z [resend.com](https://resend.com) + `EMAIL_FROM` = nadawca na **zweryfikowanej w Resend domenie** (np. `E-Forge <raport@empire-forge.com>`). Bez nich **zero wysyłek** (cron `weekly-digest` zwraca `{skipped:true}`, paragony no-opują). ⚠️ **Ustawienie tych dwóch zmiennych włącza JEDNOCZEŚNIE:** (1) tygodniowe digesty do **właścicieli portali** (#773) **oraz** (2) **paragony/podziękowania do WIDZÓW** po zaksięgowanej donacji (PayMedia · Streamlabs · ręczne dopasowanie w adminie). Nadawca jest **globalny dla całej platformy** (nie per-portal) — widz obcego portalu dostanie maila od `EMAIL_FROM`, więc użyj neutralnego nadawcy platformy albo poczekaj na nadawców per-tenant |
...
| `VERCEL_ENV` · `NEXT_RUNTIME` | Auto-ustawiane przez Vercel/Next (`production`/`preview`/`development`, `nodejs`/`edge`) — odczytywane w kodzie, nie ustawiasz ręcznie. `VERCEL_ENV` klasyfikuje deploy (`lib/deployment.ts`) i **bramkuje cron donacji** `/api/cron/streamlabs-poll`: na deployu `preview`/`development` cron nie poluje i nie wysyła alertu Sentry (zwraca 200 `skipped:"non-production-deployment"`), żeby fałszywy alarm z podglądu nie znieczulał na realny zastój wpływu. Bramka jest **fail-open** — brak zmiennej (portal self-hosted, Docker, inny host) albo nieznana wartość = traktowane jak produkcja, więc polling i alerting działają tam **bez zmian**. Ta sama zmienna ustawia `environment` w Sentry (`sentry.server.config.ts`) |
