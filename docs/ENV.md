...
| `RESEND_API_KEY` / `EMAIL_FROM` (O) | **Dormant** email transakcyjny (#773 — tygodniowe digesty do właścicieli portali). `RESEND_API_KEY` z [resend.com](https://resend.com) + `EMAIL_FROM` = nadawca na **zweryfikowanej w Resend domenie** (np. `E-Forge <raport@empire-forge.com>`). Bez nich cron `weekly-digest` zwraca `{skipped:true}` — zero wysyłek |
