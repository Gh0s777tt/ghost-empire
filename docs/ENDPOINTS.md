...
| `…/api/cron/backup` | Cron (Vercel, 05:00) — off-site backup JSON → bucket S3-compatible (R2/B2/S3). **Uśpiony** bez `BACKUP_S3_*` (`{ skipped: true }` + 200 — brak konfiguracji to stan zamierzony), ale uśpienie **widać w liście kontrolnej panelu** (krok `backup`), więc nie jest ciche. **Nieudana wysyłka zwraca 503**, nie 200 — monitoring cronów patrzy na kod odpowiedzi, a kopia, o której myślisz, że jest, ale jej nie ma, to najgorszy rodzaj cichej awarii (`CRON_SECRET`). |
...
| `…/api/admin/economy-collusion` | GET — skan zmów/multikont. Doszły sygnały **jednorazowej domeny e-mail** i **skupiska kont na jednej domenie** (`lib/disposable-email`, opcjonalnie wzbogacone przez Disify). Patrzy wyłącznie na DOMENĘ, nigdy na adres; fail-open. |
