...
- **Przełączniki funkcji (per-portal switchboard):** DRUGA oś na planach — właściciel włącza/wyłącza funkcje SWOJEGO portalu w panelu „Funkcje" (kolumna `Tenant.featureFlags Json?`), **w ramach** tego, co plan już odblokował. Flaga tylko **ZAWĘŻA** (nigdy nie poszerza planu). Jedno źródło prawdy `lib/feature-catalog.ts` (`FEATURE_CATALOG` + `isFeatureLive` = flaga **AND** plan; plan-core wydzielony do `entitlements-core.ts` bez `next/server`, żeby katalog był client-safe, re-eksport z `entitlements.ts`). Bramkuje w dwóch warstwach: **sekcje admina** (jeden filtr w `AdminClient`, `enabledFeatures` liczone serwerowo w `admin/page.tsx`) i **powierzchnie widza** (`lib/feature-gate.ts`: `isFeatureLiveForRequest` → `notFound()`, `featureFlagGateResponse` → 403; **fail-open** jak entitlements). Brak flagi / `NULL` ⇒ coded default (prawie wszystko ON; `penalties` OFF — brama prawna realnych pieniędzy zostaje), więc niezmigrowany deploy / nowy portal **nic nie chowają**. Zapis: `/api/admin/feature-flags` (tenant-scoped, walidacja katalogiem, audyt).
