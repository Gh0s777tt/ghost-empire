...
- **Auto-linkowanie po e-mailu (bezpieczeństwo).** `allowDangerousEmailAccountLinking: true` scala nowy login OAuth z istniejącym kontem o tym samym e-mailu — kluczując po `profile.email`, który trafia też do `User.email` (klucz w drugą stronę). Żeby to nie było wektorem przejęcia konta, `profile.email` jest niepusty **tylko dla e-maila zweryfikowanego przez providera** (`lib/oauth-email.ts` → `linkableEmail`/`asVerifiedFlag`): Twitch — **wyłącznie** e-mail z id_token (nigdy Helix `/users`, które zwraca adres bez względu na weryfikację), Discord — bramka po `verified`, Google — po `email_verified`, **Kick — nigdy** (patrz niżej). E-mail niezweryfikowany/brakujący → `null` → osobne konto (login się udaje, bez auto-scalania); flaga zostaje włączona, bo jej wyłączenie twardo blokowałoby login (`OAuthAccountNotLinked`). Provider `profile().id` = `providerAccountId`, więc customowe `profile()` zachowują dokładnie dotychczasowe id (Twitch `sub`, Discord `id`, Google `sub`).
- **Kick nigdy nie auto-linkuje po e-mailu (`trustWhenUnsignalled: false`).** Wcześniej ufaliśmy weryfikacji Kicka przy rejestracji — to ryzyko rezydualne zostało sprawdzone w oficjalnej dokumentacji (docs.kick.com, 2026-07-24) i **nie da się go domknąć**: Kick to **czysty OAuth 2.1, nie OIDC** (brak `id_token`, brak scope'a `openid`, brak claimów userinfo — `id.kick.com/.well-known/openid-configuration` zwraca 404), `GET /public/v1/users` oddaje **wyłącznie** `user_id`/`name`/`email`/`profile_picture`, **żaden z 11 udokumentowanych scope'ów** nie wystawia statusu weryfikacji, a token introspect zwraca tylko `active`/`client_id`/`scope`/`token_type`/`exp`. Kick bramkuje weryfikacją e-maila **streamowanie i czat** (konto niezweryfikowane opisuje jako „ograniczone"), ale **nigdzie nie deklaruje**, że blokuje samą autoryzację OAuth — więc konto z niezweryfikowanym adresem prawdopodobnie ją przejdzie. Bez dowodu własności per-login zaufanie do rejestracji pozwoliłoby atakującemu założyć konto Kick na adres ofiary i **auto-scalić je z jej kontem Google/Discord/Twitch** (przejęcie). Dlatego login przez Kicka tworzy **osobne** konto; użytkownik łączy je sam w uwierzytelnionym flow `/profile` → połączenia. **Logowanie działa bez zmian.** Gdy Kick wystawi flagę weryfikacji: dopisać ją do `KickProfile` i podać `asVerifiedFlag(...)` zamiast `undefined` (test forward-compat już to pinuje).
...
- **Bootstrap właściciela portalu (onboarding, zdarzenie `createUser`) — nadaje admina po e-mailu, więc bramka weryfikacji dotyczy go tak samo.** Nowe konto, którego e-mail równa się `Tenant.ownerEmail`, dostaje `isAdmin: true` na tym portalu. Ponieważ kluczem jest `User.email`, a ten bierze się z `linkableEmail()` (wyżej), **niezweryfikowany e-mail nigdy nie odpali tej ścieżki** — i bardzo dobrze: inaczej wystarczyłoby założyć u providera bez weryfikacji konto na adres `ownerEmail` streamera, żeby zostać **adminem jego portalu** (eskalacja uprawnień, nie tylko scalenie konta). Praktyczna konsekwencja dla onboardingu: **pierwsze logowanie właściciela musi iść przez providera z potwierdzonym e-mailem (Google / Discord / Twitch) — sam Kick nie nada admina**, bo Kick nie dostarcza żadnego sygnału weryfikacji. Właściciel bez admina: zaloguj się providerem z e-mailem albo ustaw `isAdmin` w bazie ręcznie; Kicka dopnij później w `/profile` → połączenia.
...
- **Host-routing:** edge-proxy (`middleware`) czyta `Host`, wyłuskuje slug z subdomeny (`<slug>.ROOT_DOMAIN`) przez `resolveTenantSlug` (`lib/tenant-host.ts` — czysta funkcja, bez Prisma, bezpieczna na edge) i przekazuje go nagłówkiem `x-tenant-slug`. **Rozwiązanie tenanta jest WYŁĄCZNIE z `Host`** — nagłówek jest forge'owalny (trasy `/api/*` omijają proxy), więc serwer mu nie ufa do resolucji; proxy go nadto strippuje i ustawia od nowa (defense-in-depth). Kolejność w `getCurrentTenant` (`lib/tenant.ts`): (1) subdomena → po `slug`; (2) gdy brak subdomeny (apex / `www` / pełna własna domena) → tenant, którego pole **`Tenant.domain`** (`@unique`) pasuje do hosta znormalizowanego przez `customDomainFromHost` (#653 — white-label na czystym apexie, np. `empire-forge.com`); (3) inaczej → tenant domyślny. Bez `NEXT_PUBLIC_ROOT_DOMAIN` subdomeny są no-opem, ale mapowanie `domain` działa niezależnie.
...
- **Branding per tenant:** nazwa/skrót/kolor/logo/nazwa+symbol tokenu/owner-handle z modelu `Tenant`; kolor marki idzie do CSS-var `--brand-rgb` (`hexToRgbTriplet`), OG-image generowany per tenant (`/api/og`). Dotyczy to też **etykiety kwoty w alertach na overlayu** (`StreamAlert.amountLabel`): kwoty w realnej walucie noszą `tenant.tokenSymbol` z `getCurrentTenant()` (nigdy literalnego „GT" — to symbol tenanta-założyciela, #801), a kwoty w **żetonach kasyna** zawsze `CHIP_SYMBOL` (🪙) z `lib/chips.ts` — żetony są wspólne dla całej platformy i celowo **nie** podlegają white-labelowi (`docs/CHIPS-CASINO.md`). Ta sama reguła obowiązuje **wiadomości bota na czacie** (`/api/bot/*` zwraca gotowy `message`, którego bot **nie** przepisuje): tekst wychodzi na czat streamera, więc bierze `tokenSymbol`/`shortName` z `getCurrentTenant()` — nigdy literalnego „GT"/„Ghost Empire" (#802). W trasach bota to **darmowy** odczyt: `getCurrentTenantBotAuth()` i `getCurrentTenant()` dzielą ten sam `cache()`owany wiersz tenanta (`botSecret` świadomie NIE wchodzi do `TenantBrand`, dlatego są to dwa wywołania, a nie jedno).
...
- **CI = GitLab CI/CD, plik `.gitlab-ci.yml` (425 linii) — i tylko on.** GitHub Actions **nie działa** (jest wyłączone na koncie właściciela; GitHub to ręcznie odświeżany, read-only mirror), a katalog `.github/workflows/` **nie istnieje** — w `.github/` leży wyłącznie `dependabot.yml`, którego PR-y i tak nie trafiają do GitLaba, dlatego aktualizacje zależności robi job `renovate`. Ten punkt opisywał wcześniej pipeline GitHub Actions z jobami `quality` / `integration · postgres`, których **nie ma w tym repo**: pierwszy plik nagłówka `.gitlab-ci.yml` mówi wprost „NIE dodawać niczego w `.github/workflows/`". **7 etapów, 14 jobów:**
  ...
  - **`security`** — `gitleaks` (sekrety), `semgrep` (SAST), `trivy` (CVE zależności) — **wszystkie trzy to twarde bramki** (`allow_failure` zdjęte w audycie 2026-08; wcześniej semgrep/trivy były doradcze) — oraz `renovate` (dedykowany harmonogram, otwiera MR-y w GitLabie). Uwaga na `trivy fs`: **jeden cel na wywołanie**, po jednym przebiegu na `ghost-empire-web` i `ghost-empire-chat` (szczegóły i triaż — `MAINTENANCE.md` §6). Świadomie **bez** szablonów `Security/*.gitlab-ci.yml` — obrazy przypięte wersjami, uzasadnienie w nagłówku pliku.
