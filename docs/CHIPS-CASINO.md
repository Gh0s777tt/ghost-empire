...
- ✅ **Faza 5 (etykiety) — ściąga komend czatu (#802)** — popover „Komendy" (`CommandHelp` ← `lib/command-reference.ts`) opisywał `!slots`/`!duel`/`!heist` jako stawkę **w GT**, choć `gt-games`/`duels`/`heist` ruszają tylko `chips`: darmowa gra reklamowana jako kosztująca realną walutę (na cudzym portalu dodatkowo walutą założyciela). Klucze `commandHelp.cmd_bet`/`cmd_slots`/`cmd_duelOpen`/`cmd_heist` → „żetony"/„chips"; **świadomie NIE `%tokenName%`** — podstawienie tokena zamieniłoby leak brandu na fałszywą wycenę darmowej waluty. Namespace mają tylko `pl`/`en`, reszta renderuje fallback EN, więc poprawka EN pokrywa wszystkie 14 języków. +16 testów (`command-reference.test.ts`) skanujących **każdy** locale z tym namespace — bramka na przyszły backfill i18n. tsc+eslint+build.
- ✅ **Faza 5 (etykiety) — panel koła + tour + FAQ asystenta (#802)** — audyt 14 locale wykrył tę samą klasę błędu na 4 kolejnych powierzchniach (**86 stringów**): `admin.wheel.costLabel`/`intro`/`statSpent`/`titleReward`, `tour.steps.wheelSpinBody`, `tour.steps.tokensBody`, `assistant.faq.casino.a`. Krytyczny jest panel admina — streamer **ustawia** tam koszt spinu i nagrody, a `lib/wheel.ts` zdejmuje `chips` i księguje `currency:"CHIPS"` za spin **i** za wygraną. `tokensBody` zachowuje `%gt%` (opisuje realne saldo), ale przestał twierdzić, że GT wydaje się „w kasynie i na kole" — teraz ujawnia osobną, darmową walutę. Terminologia z `terms.s3i7` każdego locale; etykiety jednostkowe → samo 🪙 (`CHIP_SYMBOL`, wspólny dla platformy). `terms.s3*` **celowo nietknięte** (kontrastują obie waluty). +16 testów (razem 32). tsc+eslint+build.
...
- ✅ **Faza 0 (framing)** — zdjęty kurs „1 PLN=100 GT" z `about`. ⚠️ **Zdezaktualizowane 2026-07-26:** „mechanizm mintu GT z donacji zostaje" **przestało obowiązywać** — właściciel uznał `REGULAMIN_GHOST_TOKENS.md` za wiążący, a jego §7 ust. 3 zakazuje naliczania GT za wpłatę kategorycznie i niederogowalnie (§28 ust. 2). Mint jest wyłączony (#808).
