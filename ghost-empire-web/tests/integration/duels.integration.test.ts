...
describe("duels (integration, real DB)", () => {
  ...
  it("createDuel opens a pending duel WITHOUT charging the challenger (escrow is on accept)", async () => {
    ...
    expect(await chipsOf(a.id)).toBe(start); // not charged yet
  ...
  it("accept charges both stakes and pays the winner — chips conserved minus the rake", async () => {
    const a = await createUserWithChips(start);
    ...
    const aAfter = await chipsOf(a.id);
    const bAfter = await chipsOf(b.id);
    const { rake, winnerTakes } = duelPayout(bet);
    // No chips created: total after == total before - rake (the rake is burned).
  ...
  it("a second accept finds no pending duel — no double payout", async () => {
    const a = await createUserWithChips(start);
    const b1 = await createUserWithChips(start);
    const b2 = await createUserWithChips(start);
    await createDuel(ch(a.id));
    ...
    expect(await chipsOf(b2.id)).toBe(start); // untouched
  });
  ...
  it("rejects accept when the accepter can't afford the bet (both stay untouched)", async () => {
    ...
    expect(await chipsOf(poor.id)).toBe(bet - 1); // not charged
    expect(await chipsOf(a.id)).toBe(start); // challenger charge rolled back too
  });
});
