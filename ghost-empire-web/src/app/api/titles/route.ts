...
import { PROFILE_TITLES, titleById, isValidTitleId, parseOwnedTitles } from "@/lib/titles";
...
export async function GET() {
  ...
  const u = await prisma.user.findUnique({
    ...
    select: { tokens: true, ownedTitles: true, equippedTitleId: true },
  });
  ...
  if (body.action === "buy") {
    ...
    try {
      const result = await prisma.$transaction(async (tx) => {
        ...
        const u = await tx.user.findUnique({ where: { id: userId }, select: { tokens: true, ownedTitles: true } });
      ...
      if ("error" in result) {
        return result.error === "owned"
          ? NextResponse.json({ error: "Masz już ten tytuł" }, { status: 409 })
          : NextResponse.json({ error: "Za mało GT" }, { status: 402 });
      }
