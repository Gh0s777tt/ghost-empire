...
export async function GET() {
  ...
  // Public + slow-changing, but a 30-day groupBy over the high-volume casino table on
  // EVERY request. Cache the shaped result per tenant for 60s (mirrors getCachedRanking).
  // #audit-v2 perf
  const data = await cacheJson(`gt-leaderboard:${tid ?? "_"}`, 60_000, async () => {
