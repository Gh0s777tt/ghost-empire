...
export async function POST(req: Request) {
  ...
  const game =
    ...
    body.game === "plinko" ? "plinko" :
    body.game === "scratch" ? "scratch" : null;
  ...
  const result = await playGtGame(session.user.id, game, Math.floor(Number(body.bet ?? 0)), choice, await currentTenantId());
