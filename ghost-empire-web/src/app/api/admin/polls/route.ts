...
export async function POST(req: Request) {
  ...
  switch (body.action) {
    case "create": {
      ...
      const accentColor =
        typeof body.accentColor === "string" && /^#[0-9a-fA-F]{6}$/.test(body.accentColor)
          ? body.accentColor
          : undefined;
      const poll = await prisma.poll.create({ data: { question, options, createdById: auth.userId, ...(tid ? { tenantId: tid } : {}), ...(accentColor ? { accentColor } : {}) } });
    ...
    case "reopen": {
      ...
      await prisma.poll.updateMany({ where: { id, ...(tid ? { tenantId: tid } : {}) }, data: { status } });
    ...
    case "delete": {
      const id = String(body.id ?? "");
      if (!id) return NextResponse.json({ error: "Brak id" }, { status: 400 });
      await prisma.poll.deleteMany({ where: { id, ...(tid ? { tenantId: tid } : {}) } }); // votes cascade
