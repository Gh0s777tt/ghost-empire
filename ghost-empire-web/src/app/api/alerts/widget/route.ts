...
// Polled fallback for a single custom widget (by id) — the realtime path is the
// generic SSE streamer /api/overlay/stream/widget?id=. Both share the
// lib/overlay-feeds producer so the payload is identical regardless of transport.
// Token-gated.
