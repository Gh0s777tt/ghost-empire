...
export function AlertCard({ alert, accent }: { alert: AlertCardData; accent: string }) {
  return (
    <div
      style={{
        position: "relative",
    ...
    >
      ...
      {/* Amount */}
      {alert.amount != null && (
        <div style={{ flexShrink: 0, textAlign: "right", color: accent, fontWeight: 800, fontSize: 22, lineHeight: 1 }}>
          {alert.amount.toLocaleString("pl-PL")}
          {alert.amountLabel && (
            <div style={{ fontSize: 11, color: "#a1a1aa", fontWeight: 600, marginTop: 2 }}>{alert.amountLabel}</div>
          )}
        </div>
      )}
    </div>
