...
function widgetPreview(id: string, t: TFn, tokenSymbol: string, brandColor: string): ReactNode {
  switch (id) {
    ...
    case "clan":
      return <div style={{ display: "inline-flex", alignItems: "center", gap: 10, background: "rgba(15,15,20,0.92)", borderRadius: 14, padding: "8px 14px", border: `2px solid ${brandColor}`, color: "#fff" }}><span style={{ fontSize: 30 }}>🛡️</span><div><div style={{ fontSize: 9, fontWeight: 800, letterSpacing: 1, color: brandColor }}>👑 CHAMPION CLAN</div><div style={{ fontSize: 15, fontWeight: 800 }}>[GH0ST] Widma</div><div style={{ fontSize: 11, opacity: 0.75 }}>1 250 000 GT · 👥 24</div></div></div>;
    default:
      return null;
  }
}
