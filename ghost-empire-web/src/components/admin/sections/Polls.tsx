...
export function PollsManager({
  ...
  async function create() {
    ...
    if (!question.trim()) { onToast("err", t("qRequired")); return; }
    if (options.length < 2) { onToast("err", t("optMin")); return; }
    if (await call({ action: "create", question: question.trim(), options, accentColor: newAccent }, t("created"))) {
  ...
  const previewOptions = (previewLabels.length >= 2 ? previewLabels.slice(0, 10) : (t.raw("previewOpts") as string[])).map(
    (label, i) => ({ label, count: [42, 27, 15, 9, 6][i] ?? 3 }),
  );
  const previewTotal = previewOptions.reduce((s, o) => s + o.count, 0);

  return (
    <SectionCard title={t("title")} icon={BarChart3}>
      <div className="space-y-4">
        ...
        >
          <div className="flex justify-center">
            <PollOverlayCard
              question={question.trim() || t("previewQuestion")}
        ...
        <div className="border border-zinc-800 bg-black/30 p-3 space-y-2">
          <div className="text-[10px] font-mono uppercase tracking-widest text-zinc-500">{t("newPoll")}</div>
          <FieldInput label={t("questionLabel")} value={question} onChange={setQuestion} placeholder={t("questionPh")} />
          <FieldTextarea label={t("optionsLabel")} value={optionsText} onChange={setOptionsText} />
          ...
          <div className="border bg-black/40 p-3" style={{ borderColor: newAccent }}>
            <div className="text-[10px] font-mono uppercase tracking-widest mb-1.5" style={{ color: newAccent }}>{t("previewBadge")}</div>
            <div className="text-sm text-white mb-2">{question.trim() || t("previewQEmpty")}</div>
            <div className="space-y-1">
              {(optionsText.split(/\r?\n/).map((s) => s.trim()).filter(Boolean).length >= 1
                ? optionsText.split(/\r?\n/).map((s) => s.trim()).filter(Boolean)
                : [t("optionPh", { n: 1 }), t("optionPh", { n: 2 })]
              ...
              <div key={p.id} className="border border-zinc-800 bg-zinc-950 p-3">
                <div className="flex items-start justify-between gap-2 mb-2">
                  <div className="min-w-0">
                    ...
                    <div className="text-[10px] font-mono text-zinc-500">{p.totalVotes} {t("votesWord")} · {p.status === "open" ? t("statusOpen") : t("statusClosed")}</div>
                  </div>
                  <div className="flex gap-1.5 shrink-0">
                    {p.status === "open" ? (
                      <button onClick={async () => { if (await call({ action: "close", id: p.id }, t("closed"))) await load(); }} disabled={busy === p.id}
                        className="text-[9px] font-mono uppercase tracking-widest px-2 py-1 border border-zinc-700 text-zinc-300 hover:border-zinc-500 disabled:opacity-50">{t("closeBtn")}</button>
                    ) : (
                      <button onClick={async () => { if (await call({ action: "reopen", id: p.id }, t("opened"))) await load(); }} disabled={busy === p.id}
                        className="text-[9px] font-mono uppercase tracking-widest px-2 py-1 border border-green-800 text-green-300 hover:border-green-600 disabled:opacity-50">{t("openBtn")}</button>
                    )}
                    <button onClick={async () => { if (window.confirm(t("deleteConfirm")) && await call({ action: "delete", id: p.id }, t("deleted"))) await load(); }} disabled={busy === p.id}
